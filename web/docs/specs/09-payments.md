...
| WPAY-005 | An outbound payment row MUST link to its withdrawal where one exists |
...
| WPAY-023 | Where the payment is unattributed, the drawer MUST state **which of the three attribution conditions failed**: no active order on the address, asset mismatch, or outside the assignment window (backend `ORD-020`). "Unattributed" alone tells the operator nothing about what to do next; the asset-mismatch case in particular (backend `ORD-002a` — 25 TRX sent against a 25 USDT order) has a completely different remedy from the other two |
...
| WPAY-031 | Each row MUST show the failed-condition reason from `WPAY-023` |
...
| WPAY-043 | An orphaned payment MUST show which order it had been contributing to, and that order's current status. A reorg that reverts a `paid` order to `partial` is the case that matters, and backend `CHN-014` emits `order.reverted` for it |
