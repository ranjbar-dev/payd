...
function ChainParametersCard({ params, error, reload }: Readonly<{ params?: ChainParameters; error: unknown; reload: () => void }>) {
  ...
  if (!params) return <section className="border border-border-subtle bg-panel p-4"><h2 className="font-semibold">Chain parameters</h2><p className="mt-2 text-sm text-ink-secondary">Loading live fee parameters.</p><ReadProblem error={error} reload={reload} /></section>;
...
function ResourceWalletCard({ wallet, config, error, reload }: Readonly<{ wallet?: ResourceWallet; config?: ResourceConfig; error: unknown; reload: () => void }>) {
  if (!wallet) return <section className="border border-border-subtle bg-panel p-4"><h2 className="font-semibold">Resource wallet</h2><p className="mt-2 text-sm text-ink-secondary">Loading the withdrawal path dependency.</p><ReadProblem error={error} reload={reload} /></section>;
