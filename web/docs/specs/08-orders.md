...
| WORD-023 | The assignment window MUST be shown explicitly: order `created_at` to `address_released_at`. This is the interval that decides attribution (backend `ORD-002b`), and it is otherwise invisible |
| WORD-023a | A null `address_released_at` has **two** meanings and MUST NOT be rendered as one. For a non-terminal order it means the address is still held and the window is open — render "still assigned". For a terminal order it means the address finished cooling and returned to the pool, so the release time was cleared rather than archived — render "no longer recorded" and state that attribution for this order is already settled. Rendering both as "still assigned" tells the operator a released address is still held by a dead order |
| WORD-023b | The UI MUST NOT reconstruct a missing upper bound from the address's current state. Once the address has been reassigned, its `released_at` belongs to a different order, and the backend deliberately returns null rather than that value (`INV-5`) |
...
| WORD-039 | On 503 `address_pool_exhausted` (backend `API-006`/`LIF-003`), the UI MUST explain that the address pool has reached `wallet.pool_max_size` with no free address, and link to the addresses page. It MUST NOT present this as a transient error to be retried |
...
| WORD-041 | On 400 `unknown_consumer` (backend `API-004`/`IPN-015`), the UI MUST name the consumer and link to the webhooks page. It MUST branch on that code alone: `invalid_order` covers asset and amount validation, and telling an operator their consumer is disabled when the amount was malformed sends them to the wrong page entirely |
