...
| 24-system | PAGE | IN_PROGRESS | 1 | Spawned 2026-08-21 via Agent tool (general-purpose). Orchestrator tightened `/config`'s response schema in `openapi.yaml` before spawning: it was `additionalProperties: true` on `assets`/`withdrawal`/`tron`/`orders`, and the `resources` block the backend's `effectiveConfig` handler has emitted since `21-resources` was never documented in the schema at all (`required` array didn't even list it). Both gaps closed to precise field-level schemas matching `backend/internal/api/effective_config.go` exactly, so this task doesn't rediscover them as a halt. Brief at `web/.codex/briefs/24-system.md` also points the worker at `useScopes()` (from `23-reports`) and `readinessDetail` (from `09-overview`) to reuse rather than reinvent. |
