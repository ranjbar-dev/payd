...
function Field({ label, children, numeric = false }: Readonly<{ label: string; children: ReactNode; numeric?: boolean }>) {
  return <div><dt className="text-ink-faint text-[11px] uppercase">{label}</dt><dd className={`mt-1 break-words ${numeric ? "text-right font-mono tabular-nums" : ""}`}>{children}</dd></div>;
}
...
function ReadError({ error, updatedAt, reload }: Readonly<{ error: unknown; updatedAt: number; reload: () => void }>) {
  ...
  return <ErrorState error={{ code: paydError?.code ?? "upstream_unreachable", details: paydError?.details }} copyByCode={copyByCode} lastUpdatedAt={updatedAt || undefined} pollingIntervalMs={DETAIL_INTERVAL} onRetry={reload} />;
}

function AmbiguousOutcome({ withdrawal }: Readonly<{ withdrawal: Withdrawal }>) {
  const tronscanBaseUrl = useTronscanBaseUrl();
  return <section className="card needs-operator" aria-label="Ambiguous outcome"><div className="flex items-center gap-2"><StatusBadge status="needs_operator" /><h2 className="card-title text-severity-critical">Outcome requires an operator decision</h2></div><ol className="mt-3 list-decimal space-y-2 pl-5 text-[13px]"><li>The funds may or may not have moved.</li><li>{withdrawal.txid ? <>Check this transaction on Tronscan: <TxidLink txid={withdrawal.txid} tronscanBaseUrl={tronscanBaseUrl} />.</> : "No transaction ID was recorded; the chain cannot be checked from this record."}</li><li>Last lookup error: <code className="select-all font-mono">{withdrawal.last_lookup_error || "—"}</code>.</li><li>The service will attempt nothing further.</li><li>Recording an outcome is a decision record, not an action.</li></ol></section>;
}

function Resources({ withdrawal }: Readonly<{ withdrawal: Withdrawal }>) {
  const expensive = withdrawal.energy_source === "burned";
  return <section className="card"><h2 className="card-title">Resource and fee breakdown</h2><dl className="mt-3 grid gap-4 sm:grid-cols-2 xl:grid-cols-4"><Field label="Energy source">{expensive ? <span className="inline-flex items-center gap-1 text-severity-warning"><AlertTriangle aria-hidden="true" size={14} strokeWidth={1.75} />burned — expensive path</span> : <span className="font-mono">{withdrawal.energy_source || "—"}</span>}</Field><Field label="Energy cost" numeric><Amount value={withdrawal.energy_cost_trx} asset="TRX" /></Field><Field label="Energy used" numeric>{withdrawal.energy_used}</Field><Field label="Bandwidth source"><span className="font-mono">{withdrawal.bandwidth_source || "—"}</span></Field><Field label="Bandwidth cost" numeric><Amount value={withdrawal.bandwidth_cost_trx} asset="TRX" /></Field><Field label="Chain fee" numeric><Amount value={withdrawal.network_fee_trx} asset="TRX" /></Field><Field label="Fee raw (SUN)" numeric><code className="select-all font-mono">{withdrawal.fee_raw}</code></Field><Field label="Resource fee" numeric><Amount value={withdrawal.resource_fee_trx} asset="TRX" /></Field><Field label="Total cost" numeric><Amount value={withdrawal.total_cost_trx} asset="TRX" /></Field></dl></section>;
}

function LoadingState() {
  return <div className="space-y-4" aria-label="Loading withdrawal detail"><section className="card animate-pulse"><div className="h-3 w-28 bg-border-subtle" /><div className="mt-4 grid gap-4 sm:grid-cols-2 xl:grid-cols-3">{Array.from({ length: 15 }, (_, index) => <div key={index}><div className="h-3 w-20 bg-border-subtle" /><div className="mt-2 h-4 w-32 bg-border-subtle" /></div>)}</div></section><section className="card animate-pulse"><div className="h-3 w-36 bg-border-subtle" /><div className="mt-4 h-28 w-full bg-border-subtle" /></section></div>;
}
...
export function WithdrawalDetail({ id }: Readonly<{ id: string }>) {
  ...
  return <main className="page"><header><p className="page-kicker"><Banknote aria-hidden="true" size={14} strokeWidth={1.75} />Operations / Withdrawals / Detail</p><div className="mt-1 flex flex-wrap items-center gap-3"><h1 className="page-title">Withdrawal <EntityId value={withdrawal.id} full /></h1><StatusBadge status={withdrawal.status} /></div></header>
    {withdrawal.status === "needs_operator" ? <><AmbiguousOutcome withdrawal={withdrawal} /><WithdrawalResolve withdrawal={withdrawal} /></> : null}
    <section className="card"><h2 className="card-title">Withdrawal record</h2><dl className="mt-3 grid gap-4 sm:grid-cols-2 xl:grid-cols-3"><Field label="Full withdrawal ID"><EntityId value={withdrawal.id} full /></Field><Field label="Status"><StatusBadge status={withdrawal.status} /><Elapsed status={withdrawal.status} updatedAt={withdrawal.status_updated_at} /></Field><Field label="Status entered" numeric><Timestamp seconds={withdrawal.status_updated_at} /></Field><Field label="Asset">{withdrawal.asset}</Field><Field label="Amount" numeric><Amount value={withdrawal.amount} asset={withdrawal.asset} /></Field><Field label="USD snapshot" numeric><Amount value={withdrawal.amount_usd} asset="USD" variant="usd-snapshot" /></Field><Field label="Source address">{address(withdrawal.from_address)}</Field><Field label="Destination address">{address(withdrawal.to_address)}</Field><Field label="Transaction ID">{withdrawal.txid ? <TxidLink txid={withdrawal.txid} tronscanBaseUrl={tronscanBaseUrl} /> : <span className="text-ink-faint">—</span>}</Field><Field label="Created" numeric><Timestamp seconds={withdrawal.created_at} /></Field><Field label="Broadcast" numeric><Timestamp seconds={withdrawal.broadcast_at} /></Field><Field label="Confirmed" numeric><Timestamp seconds={withdrawal.confirmed_at} /></Field><Field label="Failure reason"><code className="select-all font-mono text-sm">{withdrawal.failure_reason || "—"}</code></Field><Field label="Resolved by"><span className="font-mono">{resolvedBy(withdrawal.resolved_by)}</span></Field><Field label="Last lookup error"><code className="select-all font-mono text-sm">{withdrawal.last_lookup_error || "—"}</code></Field></dl><div className="mt-4"><p className="text-ink-faint text-[11px] uppercase">Raw broadcast response</p><pre className="mt-1 max-h-80 overflow-auto border border-border-subtle bg-inset p-3 text-xs text-ink-secondary"><code className="select-all font-mono">{withdrawal.broadcast_response || "—"}</code></pre></div></section>
    <Resources withdrawal={withdrawal} />
    <section className="card"><h2 className="card-title">Related records</h2><div className="mt-3 flex flex-wrap gap-x-5 gap-y-2 text-[13px]"><Link href={`/addresses/${encodeURIComponent(withdrawal.from_address)}`} className="cursor-pointer text-severity-progress underline underline-offset-2 transition-colors duration-150 hover:text-ink focus-visible:outline-2 focus-visible:outline-[var(--focus-ring)]">Source address</Link>{withdrawal.txid ? <Link href={`/payments?txid=${encodeURIComponent(withdrawal.txid)}`} className="cursor-pointer text-severity-progress underline underline-offset-2 transition-colors duration-150 hover:text-ink focus-visible:outline-2 focus-visible:outline-[var(--focus-ring)]">Outbound payment ledger row</Link> : <span className="text-ink-secondary">Outbound payment ledger row is available once a transaction ID is recorded.</span>}<Link href={`/resources?withdrawal_id=${resourceQuery}`} className="cursor-pointer text-severity-progress underline underline-offset-2 transition-colors duration-150 hover:text-ink focus-visible:outline-2 focus-visible:outline-[var(--focus-ring)]">Energy purchase record</Link><Link href={`/resources?tab=grants&withdrawal_id=${resourceQuery}`} className="cursor-pointer text-severity-progress underline underline-offset-2 transition-colors duration-150 hover:text-ink focus-visible:outline-2 focus-visible:outline-[var(--focus-ring)]">Resource grants</Link></div></section>
