...
| WIPN-032 | Where an event is order-scoped, the row MUST link to the order and show that order's current status alongside the snapshot, mirroring the `current_status` field the dispatcher adds at send time |
