...
═══════════════════════════════════════════════════════════════════════
REMEDIATION — read this last. Your previous run is on disk and is accepted.
Change ONE thing.
═══════════════════════════════════════════════════════════════════════

Everything you built passes: tsc, lint, `npm test` 4/4, and the production build.
The force-cancel second-confirmation flow, the resolve dialog wording, and the
create form's error branches are all correct and stay as they are. Do not
restructure anything.

FAILURE:

  WORD-041 — you branched on `invalid_order` to report an unknown or disabled
  consumer, at `app/(dash)/order-create-form.tsx:42`. Your reading of the code
  was right at the time: `ErrUnknownConsumer` did map to `invalid_order`. But so
  does asset and amount validation, on the same endpoint — so that branch tells
  an operator with a malformed amount that their CONSUMER is disabled, and sends
  them to the webhooks page to fix something that is not broken.

  The backend now separates them. An unknown or disabled consumer returns 400
  `unknown_consumer` — the same code `/ipn/test` already uses for the same
  condition. `invalid_order` now means only what its name says: the asset or the
  amount is wrong.

  Change that one branch to test `unknown_consumer`, and give `invalid_order` its
  own branch that names the asset/amount problem and does NOT link to webhooks.
  `openapi.yaml` and WORD-041 are already updated; do not change either.
