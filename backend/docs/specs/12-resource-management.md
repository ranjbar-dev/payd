...
| RES-013 | The signed transaction MUST be broadcast via `POST /wallet/broadcasthex` with its full signed protobuf transaction hex and tracked in `resource_grants` with the appropriate `source` and `resource_type`. **This broadcast is subject to the withdrawal-engine spec's §13.0 no-retry rule** — it is a fund-moving transaction and MUST be attempted at most once, then resolved on-chain |
