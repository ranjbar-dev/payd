...
═══════════════════════════════════════════════════════════════════════
UNBLOCKED — read this section last.
═══════════════════════════════════════════════════════════════════════

Your previous run stopped on WWD-012 without writing any file, and it was the
right call: the requirement asked for time-in-state and the contract carried no
status-transition timestamp. Do not stop on it again.

`Withdrawal.status_updated_at` now exists — a required integer, Unix seconds,
the moment the row entered its CURRENT status. It was already stored on every
status write in the withdrawals table and simply was never serialized. It is in
`backend/internal/api/openapi.yaml` and in `withdrawalSchema` in
`web/lib/payd/schemas.ts`; both are already updated, do not change either.

WWD-012 has been rewritten to name it: elapsed time in state is measured from
`status_updated_at`, and MUST NOT be measured from `created_at`. Time since the
request says nothing about the state a withdrawal is stuck in once it has moved
through three of them.

Two things this does not change:
  - WWD-013 still stands. Show elapsed time; show NO progress percentage and NO
    ETA. The engine provides neither.
  - The elapsed figure is a duration on a timestamp, not money. INV-2 does not
    apply to it. INV-2 still applies with full force to every amount on the page.
