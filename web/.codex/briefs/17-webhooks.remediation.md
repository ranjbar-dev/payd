...
Your previous run stopped before writing a file, and the finding was correct:
`GET /ipn/dead` returned no payload, so WIPN-031, WIPN-032 and WIPN-033 were all
unbuildable. Two things have changed, and one of your two asks is answered
differently than you proposed.

  1. `payload` NOW EXISTS on every dead-letter row. It was stored in
     `ipn_outbox.payload` all along and simply never selected
     (`store/outbox.go`), which makes it the sixth field in this API that was
     recorded but never serialized. It is the event body EXACTLY as composed when
     the event was queued — an immutable snapshot, never rewritten. `openapi.yaml`
     and `deadIpnPageSchema` are both updated; do not change either.

  2. `current_status` IS NOT AND WILL NOT BE A FIELD ON THAT RESPONSE. The
     dispatcher computes it at SEND time and adds it to the outgoing body
     alongside `snapshot_age_seconds` — see `deliveryBody` in
     `internal/ipn/dispatcher.go:217`, and note the comment on the last line:
     "IPN-021a: only these two fields are added to the stored snapshot." Storing
     it would make the snapshot mutable, which is the one property it exists to
     have.
     So WIPN-032 is satisfied by fetching the ORDER: for an order-scoped event,
     read the current status from `GET /orders/{id}` and show it beside the
     snapshot. WIPN-032 has been reworded to say this explicitly.

  WIPN-033 then falls out: when the snapshot's status and the order's current
  status disagree, FLAG IT AS EXPECTED, not as corruption. An `order.paid`
  snapshot sitting next to an order that is now `partial` is a reorg, and backend
  IPN-025 requires consumers to handle exactly that. Comparing two status strings
  for the purpose of displaying that contradiction is rendering, not business
  logic — INV-5 is not in your way here. What you must NOT do is decide from it
  whether the event should be redelivered.

  A payload contains order and payment data. It contains no secret, and no
  consumer secret exists anywhere in this response — WIPN-011 and WIPN-054 are
  unaffected and still absolute.

Now build the whole task as briefed above.

═══════════════════════════════════════════════════════════════════════
REMEDIATION — read this last. Your work is accepted. Change ONE thing.
═══════════════════════════════════════════════════════════════════════

Everything you built passes: tsc, lint, `npm test` 4/4, the production build, and
`/webhooks` is routed. The dry-run gating is better than the requirement asked
for — tying the acknowledgement to a filter signature so changing the range
invalidates it is exactly right. The WIPN-033 contradiction notice, the absence
of any replay loop, and the absence of any consumer secret all verify. Do not
restructure any of it.

FAILURE — WP2 gate G2-5, and it is a gate failure, not a nitpick:

  `DAT-041` requires every mutation to invalidate its entity, every list
  containing it, AND THE ALARM COUNTERS. Your single dead-letter retry
  (`webhook-dead-letters.tsx`) calls `onRetrySuccess`, which
  `webhooks-dashboard.tsx:43` implements as `await dead.refetch()` — the list
  only.

  So after an operator retries the last dead IPN, the dead-letter table empties
  but the nav alarm counter still shows it. That counter reads `GET /stats`, and
  nothing invalidated it. The operator is looking at an alarm for work they have
  already done, on the one page whose whole purpose is clearing that alarm.

  Every other mutation in the dashboard already does this correctly — see
  `order-actions.tsx`, `payment-attribute.tsx`, `address-disable.tsx`, and your
  own bulk replay in `webhook-replay.tsx:32`, which invalidates
  `queryKeys.stats()` alongside the dead list. The single retry is the one that
  does not.

  Fix: on a successful single retry, invalidate `queryKeys.stats()` as well as
  refetching the dead-letter list. Match what your own replay path already does.

  WIPN-036 still stands: RE-FETCH the row, never optimistically update it. This
  change adds an invalidation; it must not add an optimistic write.
...
where the stats invalidation now happens.
