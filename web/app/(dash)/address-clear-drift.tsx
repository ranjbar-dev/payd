...
export function AddressClearDrift({ address, asset, chainRaw }: Readonly<{ address: string; asset: string; chainRaw: string }>) {
  ...
  return <>
    <button type="button" className="mt-3 border border-severity-critical bg-[var(--severity-critical-bg)] px-3 py-2 text-sm font-medium text-severity-critical focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-2" onClick={show}>Acknowledge drift for {asset}</button>
    <ConfirmDialog open={open} onClose={() => !mutation.isPending && setOpen(false)} title={`Acknowledge ${asset} balance drift`} confirmLabel="Record acknowledgement" requiresTotp ready={acknowledged} error={error} onConfirm={async (totp) => {
      ...
      <p className="mt-3">Investigate before clearing: drift usually means a payment the detector missed or a transfer nothing recorded. Review the <Link href={`/payments?address=${encodeURIComponent(address)}`} className="text-severity-progress underline underline-offset-2">payment history</Link> and <a href={tronscan} target="_blank" rel="noreferrer" className="text-severity-progress underline underline-offset-2">Tronscan address</a>.</p>
