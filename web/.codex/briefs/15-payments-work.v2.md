...
Your previous run stopped on WPAY-043 without writing a file. You read the
contract correctly and you were right to refuse to infer an order from the
address and asset — that would have named an order the backend never credited.

But the DOCUMENTATION was wrong, not the data. `RewindChain` orphans a payment
with `UPDATE payments SET status='orphaned' WHERE block_height > ?`
(`backend/internal/store/follower.go:418`). It sets the status and NOTHING ELSE:
the attribution stays exactly where it was. So an orphaned payment that had been
attributed still carries its `order_id`, and the orphaned listing returns it.

`openapi.yaml` has been corrected — `order_id` is null on the UNATTRIBUTED
listing, and on the orphaned listing only when the payment was never attributed
in the first place. WPAY-043 has been rewritten to match:
  - read the order from the payment's own `order_id`;
  - fetch that order's CURRENT status from `GET /orders/{id}` — current, not the
    status it had when the payment was orphaned, because the whole point is
    seeing that a `paid` order is now `partial`;
  - where `order_id` IS null, say the payment was never attributed. Do not
    search for a candidate order.

Nothing else changes. WPAY-044 still stands with full force: no restore,
re-confirm, recover or re-detect control anywhere on that page.
