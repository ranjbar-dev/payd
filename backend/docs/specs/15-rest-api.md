...
| API-043 | `GET /config` | `admin:read` | MUST return only allowlisted asset, withdrawal, chain-depth, order-TTL, energy, price, wallet-pool, and consumer-name fields; it MUST have no field capable of containing endpoint/API/TOTP/key-hash/consumer credentials (CFG-011), preventing environment mistakes without creating a secret endpoint. **The allowlist MUST include the operator-visible thresholds** `energy.max_burn_trx`, `energy.balance_warn_trx`, `price.stale_after_seconds`, `wallet.pool_min_free`, `wallet.pool_max_size`, and `wallet.cooldown_seconds`. Each is a number, duration, or decimal string and so is structurally incapable of carrying a credential; the projection is explicit rather than a redaction pass, which is what keeps that guarantee true as config grows. Without them a dashboard can show a live figure but not how close it is to the limit that will reject it — `getEnergyFee` against the burn ceiling (ENR-017), price age against `stale_after` (ORD-009, WDR-006b), free addresses against `pool_max_size` (LIF-003) — and the only alternative is a hardcoded second copy of the operator's own configuration, which is wrong the moment it changes |
