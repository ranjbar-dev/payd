...
| 25-polish | PAGE | IN_PROGRESS | 1 | Spawned 2026-08-21 via Agent tool (general-purpose). Orchestrator pre-audited UI-073 before writing the brief: `withdrawals-dashboard.tsx`'s list table has `hidden lg:block` with no matching card view (confirmed missing), `withdrawal-detail.tsx` has no responsive treatment at all (confirmed missing); orders/payment-worklists already have correct card fallbacks (confirmed present, not to be touched). Brief scopes the task to existing pages only, explicitly excludes the WP3-audited withdrawal/address-mutation surface. |
