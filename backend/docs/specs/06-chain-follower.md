...
| CHN-024a | **`/wallet/broadcasttransaction` MUST be exempt from CHN-024 entirely.** It MUST have no automatic retry under any circumstance. A broadcast has an irreversible external side effect and a lost response does not mean a lost transaction. See [`13-withdrawal-engine.md`](13-withdrawal-engine.md) §13.0 and WDR-017 |
