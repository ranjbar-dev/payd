...
| BFF-005a | **The allowlist MUST include the three unauthenticated routes — `/healthz`, `/readyz`, `/openapi.yaml` — as well as everything under `/api/v1/`.** They are served at the root rather than under `/api/v1/`, so a generator that filters on that prefix drops them silently and the proxy 404s a route the specification requires (`WOVW-010`/`WOVW-011`). The generator MUST fail the build if any of the three is missing from its output, rather than emitting a quietly incomplete list |
| BFF-005b | A public route MUST be forwarded to its root path and **without** `X-API-Key`. It takes no key and is not rate limited; sending one puts readiness polling on the same per-key budget as everything else for no benefit |
...
| BFF-013 | `AUTH-030`'s whoami check at login appears to require a session before one exists — the proxy refuses to forward without one (`BFF-006`), and `WST-011` forbids the login handler from setting the key itself. It is resolved by ordering, not by an exemption: the login handler creates the session **first**, then invokes the proxy handler **in process** carrying that session's own cookie, and invalidates the session before any `Set-Cookie` is emitted if whoami fails. No session-check bypass and no second key-setting site. The proxy handler MUST therefore be callable as a function, not only as an HTTP route |
