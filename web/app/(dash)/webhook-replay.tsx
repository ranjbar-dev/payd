...
export function WebhookReplay({ consumers }: Readonly<{ consumers: readonly { name: string }[] }>) {
  ...
  return <section className="border border-border-subtle bg-panel p-4"><h2 className="text-lg font-semibold">Bulk replay</h2><p className="mt-1 text-sm text-ink-secondary">Recovery is capped at {CALL_LIMIT} events per call. This page never loops: every additional replay is a separate operator action.</p>
    <p className="mt-3 border-l-2 border-severity-progress bg-inset p-3 text-sm"><strong>IPN redelivery is safe because consumers treat <code className="font-mono">event_id</code> as an idempotency key. This is the only retry in the system.</strong> Replay sends notifications again; it never changes a withdrawal or any underlying payment state.</p>
    <div className="mt-3 grid gap-3 md:grid-cols-4"><label className="grid gap-1 text-xs text-ink-secondary">Consumer filter<select value={consumer} onChange={(event) => set("replay_consumer", event.currentTarget.value)} className="border border-border-strong bg-panel px-2 py-1.5 text-sm text-ink"><option value="">All consumers</option>{consumers.map((item) => <option key={item.name} value={item.name}>{item.name}</option>)}</select></label><label className="grid gap-1 text-xs text-ink-secondary">From (local time)<input type="datetime-local" value={from} onChange={(event) => set("replay_from", event.currentTarget.value)} className="border border-border-strong bg-panel px-2 py-1.5 text-sm text-ink" /></label><label className="grid gap-1 text-xs text-ink-secondary">To (local time)<input type="datetime-local" value={to} onChange={(event) => set("replay_to", event.currentTarget.value)} className="border border-border-strong bg-panel px-2 py-1.5 text-sm text-ink" /></label><label className="flex items-end gap-2 pb-2 text-sm"><input type="checkbox" checked={dryRun} onChange={(event) => set("replay_dry_run", event.currentTarget.checked ? "" : "false")} />Dry run (default)</label></div>
    <p className="mt-2 text-xs text-ink-secondary">Resolved range (UTC, inclusive): <code className="font-mono">{utc(from)}</code> to <code className="font-mono">{utc(to)}</code>.</p>
    <div className="mt-3 flex flex-wrap gap-2"><button type="button" className="border border-border-strong px-3 py-2 text-sm hover:bg-raised disabled:opacity-50" disabled={replay.isPending} onClick={() => { replay.reset(); replay.mutate(false, { onSuccess: (result) => setChecked({ signature, count: result.count }) }); }}>Count matching dead notifications</button>{!dryRun ? <button type="button" className="border border-severity-progress px-3 py-2 text-sm hover:bg-raised disabled:opacity-50" disabled={!canReplay || replay.isPending} onClick={() => { replay.reset(); setConfirming(true); }}>Replay acknowledged range</button> : null}</div>
