...
═══════════════════════════════════════════════════════════════════════
REMEDIATION — read this last. Your wizard is accepted. ONE defect, and it is
the exact failure this page exists to prevent.
═══════════════════════════════════════════════════════════════════════

What you built is right. The idempotency key is generated once with
`setIdempotencyKey((current) => current ?? crypto.randomUUID())` and survives a
failed submission; the estimate gates step 3; the confirmation reads the six
echoed fields; the TOTP code is cleared after every error; the retry-language
scan is empty; and the ambiguous panel correctly offers NO way back to a fresh
key. Do not restructure any of it.

FAILURE — `isAmbiguous` at `withdrawal-wizard.tsx:75` treats only 502 and 504 as
ambiguous. A 500 falls through to the generic branch, which tells the operator
"payd did not report a created withdrawal" and offers "start new" — and
`startNew()` clears the idempotency key.

THAT IS A DOUBLE-PAYOUT PATH, and it is reachable:

  `store.CreateWithdrawal` (backend/internal/store/withdrawals.go:205-209) ends:
  ...
  THE ROW IS ALREADY COMMITTED when that re-read runs. If the read fails — or if
  `Commit()` itself reports an error for a commit that landed — the API renders
  a 500 through `writeWithdrawalErrorDetails`, WITH THE WITHDRAWAL WRITTEN AND
  THE TOTP CODE CONSUMED.

  Your UI then invites the operator to start a new withdrawal with a fresh key.
  The second submission is not a retry as far as the backend is concerned — new
  key, new row, second transfer. One payout becomes two, with a clean audit trail
  on both, which is exactly what WWD-005 and INV-1 exist to prevent.

FIX — precisely this, nothing more:

  1. `isAmbiguous` returns true for ANY status >= 500, not just 502 and 504. Keep
     returning true when there is no status at all (a network failure), which you
     already do correctly.
  2. Delete the generic "payd internal error" branch that offered `allowNew` on a
     5xx. Those now route to the ambiguous panel.
  3. 503 STAYS DEFINITE and keeps its current copy. It is the one 5xx that is
     genuinely safe: every 503 path — `withdrawals_disabled`, `price_unavailable`,
     `estimator_unavailable` — returns before any write. Order your checks so 503
     is handled before the >= 500 rule.

Two new requirements have been written into `web/docs/specs/11-withdrawals.md`
and are now binding: WWD-086a (every 5xx except 503 is ambiguous, with the
commit-then-read reason) and WWD-086b (the ambiguous panel must never offer a new
withdrawal, clear the key, or otherwise expose a path to a fresh one).

Then re-run `./node_modules/.bin/tsc --noEmit`, `npm run lint`, `npm test`, and
`npm run build` (move `.env` aside, then MOVE IT BACK). Report the new
`isAmbiguous` verbatim and confirm no 5xx path can reach `startNew`.

═══════════════════════════════════════════════════════════════════════
SECOND REMEDIATION — the 5xx fix is correct and accepted. ONE small gap left.
═══════════════════════════════════════════════════════════════════════

`isAmbiguous` now reads `status !== 503 && status >= 500`, 503 is handled first,
and the generic internal-error branch is gone. That is exactly right. Your note
about the 503/`startNew` instruction conflict is also right, and 503 keeping its
definite copy and its new-withdrawal path is the intended behaviour — WWD-085
says the withdrawal was not created, and every 503 path returns before any write.

FAILURE — the error chain now has no final `else`. Handled: 503, any other 5xx
(ambiguous), 409 `idempotency_key_reuse`, 409 with `totp_consumed`, 401, 429, and
400/409 with field errors. ANY OTHER STATUS FALLS THROUGH AND SETS NOTHING.

`submission` stays null, so `open={step === 3 && !submission}` keeps the dialog
open with the TOTP field cleared and no message at all. The operator has just
authorised a payout and the screen tells them nothing — not that it failed, not
that it succeeded, not what to do. On this screen that is its own kind of
dangerous: an operator who cannot tell what happened will start guessing.

It is reachable. A 403 from the CSRF check, a 404 from the proxy allowlist, or a
405 all arrive as statuses your chain does not name.

FIX — add a final `else` that sets a definite error panel:
  - state plainly that payd did NOT create the withdrawal;
  - show the status and `error.code` verbatim, selectable, so it can be quoted;
  - `allowNew: true` is correct here. Every unhandled status in this position is
    4xx and originates before payd processed anything — the proxy's own CSRF,
    allowlist and method rejections never reach the create handler, so no row can
    exist. Do NOT extend `allowNew` to anything 5xx; that is what the previous
    remediation removed.

Nothing else changes. Re-run `./node_modules/.bin/tsc --noEmit`, `npm run lint`,
`npm test`, and `npm run build` (move `.env` aside, then MOVE IT BACK). Report
the new branch verbatim.
