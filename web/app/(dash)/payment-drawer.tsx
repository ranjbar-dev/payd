...
export function PaymentDrawer({ payment, minDeposit, onClose }: Readonly<{ payment: Payment; minDeposit?: string; onClose: () => void }>) {
  ...
  return <div className="fixed inset-0 z-30 bg-black/60 p-4" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
    <div ref={dialog} role="dialog" aria-modal="true" aria-labelledby="payment-detail-title" tabIndex={-1} className="ml-auto h-full w-full max-w-2xl overflow-y-auto border border-border-strong bg-panel p-4 shadow-2xl" onKeyDown={(event) => { if (event.key === "Escape") onClose(); }}>
      <header className="flex items-start justify-between gap-4 border-b border-border-subtle pb-3"><div><p className="font-mono text-xs uppercase tracking-[0.2em] text-ink-faint">Payment detail</p><h2 id="payment-detail-title" className="mt-1 text-xl font-semibold">Payment {payment.id}</h2></div><button type="button" className="border border-border-strong p-2 hover:bg-raised" onClick={onClose} aria-label="Close payment detail"><X aria-hidden="true" size={18} /></button></header>
