...
export function WithdrawalsDashboard() {
  ...
  return <main className="mx-auto max-w-7xl space-y-4 p-4 lg:p-6"><header><p className="font-mono text-xs uppercase tracking-[0.2em] text-ink-faint">Operations / Withdrawals</p><div className="mt-1 flex flex-wrap items-center gap-3"><h1 className="text-2xl font-semibold">Withdrawals</h1>{/* WRPT-030/WRPT-036: the CURRENT list filter, straight through, not a separate export filter state. */}<ExportDialog kind="withdrawals" filters={{ status }} /></div><p className="mt-1 text-sm text-ink-secondary">Newest first as returned by payd. Needs-operator records are pinned first because their outcome is unknown.</p></header>
