...
| 26-coverage | AUDITOR | DONE | 1 | Spawned 2026-08-21 via Agent tool, subagent_type=general-purpose rather than Explore (AUTOPILOT.md §2's default for AUDITOR) — deliberate deviation: unlike `22-noretry-audit` (writes no code at all, findings-only), the task graph requires this AUDITOR to update `17-api-coverage-matrix.md` directly as its own deliverable, which needs Edit access Explore's tool restriction would block. Scope enforced entirely through the brief's own "modify only this file" instruction, verified: only that one file appears in `git status` afterward. Ten rows corrected — nine additions of a genuinely missed consumer (e.g. `/wallets` also consumed by the withdrawal wizard's known-destination check, `/chain/params` also feeding Overview's readiness card), and one real misattribution: `WOVW-052`/"Overview → volume card" was on `/stats`'s row, but the spec explicitly forbids `/stats` from backing that card and names `/reports/volume` instead — independently confirmed by reading `overview-dashboard.tsx:113,121,171`, which fetches both endpoints separately and links its "Volume today (UTC)" card to `/reports?tab=volume`. `/stats` actually backs the alarms strip, nav-shell alarm counters, and addresses pool health, none of which the matrix credited it for. Zero unconsumed routes found, zero backend gaps. One finding NOT resolved (correctly, per its own scope): `POST /withdrawals`'s TOTP enforcement is conditional server-side (`if cfg.RequireTOTP`, `withdrawals.go:73`) while the other three TOTP-gated routes enforce it unconditionally and spec WWD-072 assumes it's always on — the dashboard itself always sends a code regardless, so no client-side risk today, but a `payd` deployment with `RequireTOTP=false` would silently accept a withdrawal with no second factor. Flagged for human judgment, not a matrix error to fix. |
