...
ROLE: SCAFFOLD
TASK-ID: 01-scaffold
GOAL: Create the minimal strict Next.js App Router dashboard scaffold with server-side environment validation and the specified project layout.

READ FIRST, FULLY:
  web/AGENTS.md
  web/docs/specs/02-tech-stack.md
  backend/internal/api/openapi.yaml — sections for: GET /api/v1/auth/whoami
...
REQUIREMENTS TO SATISFY (cite each in your report with file:line):
  WST-001, WST-002, WST-003, WST-004, WST-005, WST-006, WST-007,
  WST-010, WST-011, WST-012, WST-013, WST-020, WST-021, WST-022, WST-023
...
DONE WHEN:
  ...
  - a bare dashboard root and login route compile, no `app/transactions/` directory remains, and environment validation is server-only and fails on missing/empty values, an unsafe `SESSION_SECRET`, or missing required payd scopes

YOU MUST NOT:
  - add a runtime dependency outside WST-001; only install the dependencies explicitly listed there
