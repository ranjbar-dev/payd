...
export function PaymentAttribute({ payment }: Readonly<{ payment: Payment }>) {
  ...
  // WPAY-034: an asset_mismatch payment must be attributable to the order on its
  // address even though that order expects a different asset — so the candidate
  // search is by address only. Filtering by payment.asset here (WPAY-033's
  // literal wording) would return nothing for exactly the case WPAY-034 exists
  // to guard, making the mismatch confirmation unreachable.
  const query = new URLSearchParams({ address: payment.to_address, limit: "50" });
  ...
  return <><form className="mt-2 flex flex-wrap items-end gap-2" onSubmit={(event) => { event.preventDefault(); setChoosing(true); setCursor(""); }}><label className="field min-w-56">Target address<input className="input font-mono normal-case tracking-normal" value={payment.to_address} readOnly /></label><button type="submit" className="btn btn-primary" disabled={candidates.isFetching}>{candidates.isFetching ? <Loader2 aria-hidden="true" size={14} strokeWidth={1.75} className="animate-spin" /> : <Link2 aria-hidden="true" size={14} strokeWidth={1.75} />}{candidates.isFetching ? "Finding orders" : "Attribute payment"}</button>{choosing ? <button type="button" className="btn btn-ghost" onClick={() => setChoosing(false)}><X aria-hidden="true" size={14} strokeWidth={1.75} />Cancel</button> : null}</form>
    {choosing ? <div className="mt-3 min-w-80 space-y-2 border-y border-border-subtle py-3 text-xs"><p className="text-ink-secondary">Choose an order returned by payd for this payment’s address <code className="font-mono">{payment.to_address}</code>. This payment is <code className="font-mono">{payment.asset}</code>; an order expecting a different asset is allowed but triggers an extra confirmation.</p>{candidates.data?.orders.map((candidate) => <div key={candidate.id} className="row-hover border-b border-border-subtle py-2"><div className="flex items-start justify-between gap-2"><EntityId value={candidate.id} /><StatusBadge status={candidate.status} resolution={candidate.resolution} /></div><p className="mt-1"><Amount value={candidate.amount} asset={candidate.asset} /> expected · <Amount value={candidate.received} asset={candidate.asset} /> received</p><button type="button" className="btn btn-secondary mt-2" onClick={() => pickOrder(candidate)}><Link2 aria-hidden="true" size={14} strokeWidth={1.75} />Attribute to this order</button></div>)}{candidates.data && !candidates.data.orders.length ? <p className="text-ink-secondary">No orders were returned for this address and asset.</p> : null}{candidateError ? <ErrorState error={candidateError} copyByCode={readCopy} onRetry={() => void candidates.refetch()} /> : null}<CursorPager nextCursor={candidates.data?.next_cursor} hasResults={(candidates.data?.orders.length ?? 0) > 0} onNext={setCursor} onStart={() => setCursor("")} /></div> : null}
